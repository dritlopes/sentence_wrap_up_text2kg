...
text_filepath = f'../data/processed/{corpus_name}_texts.csv' # f'../data/processed/{corpus_name}_words.csv'
level = 'text'
...
if corpus_name in ['meco','provo']:
    for text, text_id in zip(texts_df['text'].tolist(), texts_df['text_id'].tolist()):
        ...
        for i in range(1, len(words)+1):
            ...
            # pp.pprint(relik_out)
            with open(
                    f"{output_dir}/{dt_string}/output_step_{i:03d}_{model_name}_{corpus_name}_{text_id}_{threshold}_{window_size}.json",
                    "w") as f:
...
if corpus_name == 'onestop':
    ...
    if level == 'article':
        texts_df.sort_values(by=['article_batch', 'article_id', 'difficulty_level', 'paragraph_id','ianum'], inplace=True)
        for info, rows in texts_df.groupby(['article_batch', 'article_id', 'difficulty_level']):
            # if ((info[0]==2) and (info[1] > 7) and (info[2]=='Ele')) or ((info[0]==2) and (info[1] > 8) and (info[2]=='Adv')) or info[0]==3:
            if ((info[0] == 3) and (info[1] == 8) and (info[2] == 'Ele')) or ((info[0] == 3) and (info[1] == 9) and (info[2] == 'Ele')) or ((info[0] == 3) and (info[1] == 9) and (info[2] == 'Adv')):
                words = rows['ia'].tolist()
                paragraph_ids = rows['paragraph_id'].tolist()
                for i in range(1, len(words) + 1):
                    context = " ".join(words[:i])
                    # if i == len(words): print(len(words), len(context))
                    relik_out: RelikOutput = relik(context)
                    # print("=== Relik Output ===")
                    # pp.pprint(relik_out)
                    filepath = (f"{output_dir}/{dt_string}/output_step_{i:03d}_{model_name}_{corpus_name}_{info[0]}-"
                                f"{info[1]}-{paragraph_ids[i-1]}-{info[2]}_{threshold}_{window_size}.json")
                    with open(filepath, "w") as f:
                        json.dump(relik_out.to_dict(), f, indent=4)